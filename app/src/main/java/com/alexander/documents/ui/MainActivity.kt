...
import com.alexander.documents.api.DocumentsRequest
...
class MainActivity : AppCompatActivity() {
    ...
    private val documentsAdapter by lazy(LazyThreadSafetyMode.NONE) {
        DocumentsAdapter(::openDocumentDetails, ::onPopUpMenuClick)
    }
    ...
    private fun requestDocs() {
        ...
        VK.execute(DocumentsRequest(), object : VKApiCallback<List<Document>> {
            override fun success(result: List<Document>) {
                if (!isFinishing) {
                    containerView.isRefreshing = false
                    documentsAdapter.documents = result.toMutableList()
                }
            }
